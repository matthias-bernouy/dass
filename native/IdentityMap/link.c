...
ReturnCodes link(const uint8_t *key, size_t length, uint64_t value, uint64_t id_transaction)
{
    ...
    while (max_iterations--)
    {
        ...
        if (slot_state == SLOT_AVAILABLE || slot_state == SLOT_DELETED) {
            ...
            TX_RESPONSE add_action_response = add_action_to_transaction(id_transaction, IDENTITY_MAP_PROVIDER, &payload);
            if ( add_action_response != TX_RESPONSE_OK ){
                force_status(&identity_hashed_map[index].status, current_status);
                return ERR_MAX_ITERATION;
            }

            TX_RESPONSE add_dependency_response = add_dependency_to_transaction(id_transaction, identity_hashed_map[index].current_transaction_id);
            if ( add_dependency_response != TX_RESPONSE_OK ){
                force_status(&identity_hashed_map[index].status, current_status);
                return ERR_MAX_ITERATION;
            }
