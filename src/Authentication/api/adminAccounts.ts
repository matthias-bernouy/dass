...
export async function adminListAccounts(_req: Request, system: DefaultAuthentication): Promise<Response> {
    try {
...
export async function adminCreateAccount(req: Request, system: DefaultAuthentication): Promise<Response> {
    try {
...
export async function adminDeleteAccount(req: Request, system: DefaultAuthentication): Promise<Response> {
    try {
...
export async function adminUpdateRole(req: Request, system: DefaultAuthentication): Promise<Response> {
    try {
        const current = await system.getSubject(req);
        if (!current) return new Response("Unauthorized", { status: 401 });
